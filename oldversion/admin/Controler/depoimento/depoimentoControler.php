...
class depoimentoControler{
    ...
    public static function GetAlldepoimentoDB($pdo){
        ...
        $prepare = $pdo->prepare("SELECT * FROM `depoimento` where `depoimento`.`Status` like 1 ORDER BY `depoimento`.`CodDepoimento` DESC");
        $prepare->execute();
    ...
    public static function UpdatedepoimentoDB($pdo,$Array){
        ...
        $sqlquery = "UPDATE `depoimento` SET `NomeDepoente` = '$NomeDepoente', `Depoimento` = '$Depoimento', `Estrela` = '$Estrela', `LocalImagem` = '$LocalImagem' WHERE `depoimento`.`CodDepoimento` = $CodDepoimento;";	
